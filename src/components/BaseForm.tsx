'use client'
import {  DEFAULT_EXAMPLE_USER_ADDRESS, macroForwarderContract, sb712MacroContract, sbMacroContract, torexContract } from "@/constants/contracts";
import { FormEvent,  useEffect, useState } from "react"
import { getContract, prepareContractCall, readContract, sendAndConfirmTransaction, ThirdwebContract, toTokens, toUnits, toWei, ZERO_ADDRESS } from "thirdweb";
import { chain } from "@/app/chain";
import { client } from "@/app/client";
import { allowance, approve, balanceOf, decimals } from "thirdweb/extensions/erc20";
import { Account } from "thirdweb/wallets";
import { TokenIcon, TokenName, TokenProvider, TokenSymbol, useReadContract } from "thirdweb/react";
...
export const BaseForm = ({ account }: BaseFormProps) => {
    ...
    const [superTokenInContract, setSuperTokenInContract] = useState<ThirdwebContract>();
    ...
    const [underlyingTokenInContract, setUnderlyingTokenInContract] = useState<ThirdwebContract>();
    const [underlyingTokenDecimals, setUnderlyingTokenDecimals] = useState<number>();
    const [underlyingTokenBalance, setUnderlyingTokenBalance] = useState<bigint>(0n);
    const [underlyingTokenAllowance, setUnderlyingTokenAllowance] = useState<bigint>(0n);
    ...
    const fetchUnderlyingToken = async (superTokenInContract: ThirdwebContract) => {
        if(!superTokenInContract && !account) {
            console.error("superTokenInContract not initialized");
            return;
        };

        const superTokenInDecimals = await decimals({ contract: superTokenInContract  })
        const superTokenInBalance = await balanceOf({ contract: superTokenInContract, address: account?.address as string })
        ...
        const underlyingTokenDecimals = await decimals({ contract: underlyingTokenInContract })
        const underlyingTokenBalance = await balanceOf({ contract: underlyingTokenInContract, address: account?.address as string })
        const underlyingTokenAllowance = await allowance({ contract: underlyingTokenInContract, owner: account?.address as string, spender: superTokenInContract?.address as string })
    ...
    async function approveToken(event: FormEvent<HTMLFormElement>) {
        ...
        if (!superTokenInContract || !underlyingTokenInContract || !underlyingTokenDecimals) {
    ...
    async function handleStartDCA(event: FormEvent<HTMLFormElement>) {
        ...
        const upgradeAmount = upgradeAmountInput === "max" ? maxUint256 : toUnits(upgradeAmountInput, underlyingTokenDecimals as number);
        if (upgradeAmount > toUnits(underlyingTokenBalance.toString(), underlyingTokenDecimals as number)) {
            alert('Upgrade amount exceeds your balance. Please enter a lower amount.');
            return;
        }
        if (upgradeAmount > toUnits(underlyingTokenAllowance.toString(), underlyingTokenDecimals as number)) {
                    ...
                    <form className="space-y-4" onSubmit={handleStartDCA}>
                        <div className="form-control">
                            <label className="label">
                                <span className="label-text">Flowrate (tokens per Month):</span>
                            </label>
                            <input
                                type="number"
                                placeholder="0"
                        ...
                        <div className="form-control">
                            <label className="label">
                                ...
                                {pairedxTokens ? (
                                    ...
                                    <div className="flex items-center gap-2">
                                        <TokenIcon className="w-4 h-4" />
                                        Current Allocated Amount: {toTokens(superTokenInBalance, superTokenInDecimals as number)}
                                        <TokenSymbol />
                                    </div>
                                    </TokenProvider>
                                ) : (
                                ...
                                <button className="btn btn-warning mt-4" type="button" onClick={(e)=> setMaxUpgradeAmount()}>Max</button>
                            </label>
                            <input
                                type="number"
                                placeholder="0"
